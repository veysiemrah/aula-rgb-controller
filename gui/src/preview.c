...
{
    /* Marquee: sine wave expanding from center outward */
    float t = (float)p->frame * speed_mult(p->speed) * 0.15f;
    float cx = 8.5f;

    for (int i = 0; i < KEY_COUNT; i++) {
        float col = (float)f87_key_layout[i].col;
        float dist = fabsf(col - cx);
        float wave = (sinf(dist * 0.8f - t) + 1.0f) * 0.5f;
        /* Fade at edges */
        float edge = 1.0f - dist / 10.0f;
        if (edge < 0) edge = 0;
        float v = wave * edge;
        if (p->colorful) {
            float hue = fmodf(dist * 30.0f - t * 15.0f, 360.0f);
            if (hue < 0) hue += 360.0f;
